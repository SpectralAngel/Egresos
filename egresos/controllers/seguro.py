...
class Seguro(controllers.Controller):
    ...
    @error_handler(index)
    @expose()
    @validate(validators=dict(afiliado=validators.Int(), indemnizacion=validators.Int(),
                            fecha=validators.DateTimeConverter(format='%d/%m/%Y'),
    ...
    def agregar(self, afiliado, indemnizacion, **kw):
        ...
        flash(u'Se ha agregado el Seguro de Vida al afiliado {0}'.format(afiliado.id))
    ...
    def eliminar(self, seguro):
        ...
        flash(u'Se ha eliminado el Seguro de Vida al afiliado {0}'.format(afiliado.id))
